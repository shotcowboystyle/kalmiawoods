import type { SpinnerColor, SpinnerSize } from '@/components/Spinner/types';
import type { Ref } from 'vue';
import { computed } from 'vue';
import type { ButtonGradient, ButtonSize, ButtonVariant } from '../types';

export interface UseButtonSpinnerProps {
  outline: Ref<boolean>;
  size: Ref<ButtonSize>;
...
export function useButtonSpinner(props: UseButtonSpinnerProps): {
  ...
  const color = computed<SpinnerColor>(() => {
    ...
    if (props.gradient.value) {
      ...
      if (props.gradient.value.includes('blue')) return 'blue';
      if (props.gradient.value.includes('pink')) return 'pink';
      if (props.gradient.value.includes('red')) return 'red';
      return 'white';
    }
