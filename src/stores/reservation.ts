...
const isReservationModalOpen = atom<boolean>(false);
export const $isReservationModalOpen = useStore(isReservationModalOpen);
export const toggleReservationModal = () =>
	isReservationModalOpen.set(!$isReservationModalOpen.value);

const monthStartDate = atom<string>(firstDayOfCurrentMonth().toDateString());
const setMonthStartDate = action(monthStartDate, 'setMonthStartDate', (date, payload: string) =>
	date.set(payload),
);

const monthEndDate = atom<string>(lastDayOfCurrentMonth().toDateString());
const setMonthEndDate = action(monthEndDate, 'setMonthEndDate', (date, payload: string) =>
	date.set(payload),
);

const reservations = map<Record<string, Reservation | undefined>>({});
...
export const $reservations = useStore(reservations);
...
export const $disabledReservationDates = useStore($reservedDates);

export const addReservation = action(
	reservations,
	'addReservation',
	async (store, newReservation) => {
...
const activeReservationId = atom<string | null>(null);
export const setActiveReservationId = action(
	activeReservationId,
	'setActiveReservationId',
	(id, newVal) => id.set(newVal),
);

const reservation = computed(
	[reservations, activeReservationId],
	(_reservations, _reservationId) =>
		Object.values(_reservations).find((r) => r?.reservationId === _reservationId) ??
		initReservationData,
);
export const $reservation = useStore(reservation);
...
// export const updateReservationDates = action(
// 	reservationDates,
// 	'updateReservationDates',
// 	(store, value) => store.set(value),
// );

// export const updateReservationStartDate = action(
// 	reservationDates,
// 	'updateReservationStartDate',
// 	(store, value) => store.setKey('checkInDate', value),
// );

// export const updateReservationEndDate = action(
// 	reservationDates,
// 	'updateReservationEndDate',
// 	(store, value) => store.setKey('checkOutDate', value),
// );
